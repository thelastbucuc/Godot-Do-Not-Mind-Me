...
const BULLET = preload("uid://8r01qajhpm3b")
...
func can_see_player() -> bool:
	return abs(get_fov_angle()) < FOV[_state] and is_player_visible()
...
func set_label() -> void:
	var s: String = "State:%s\n" % EnemyState.find_key(_state) # or EnemyState.keys()[_state]
	s += "Fin:%s\n" % nav_agent.is_navigation_finished()
	s += "Vis:%s FOV:%.0f\n" % [is_player_visible(), get_fov_angle()]
	s += "TAR:%s CAN:%s" % [nav_agent.target_position, can_see_player()]
	debug_label.text = s
